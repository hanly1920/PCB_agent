...
def infer_region_type(comp: dict, semantic_class: str, functional_group: str, bbox: List[float]) -> Tuple[str, str]:
    ...
    if _is_edge_preference_component(comp, semantic_class):
        if semantic_class in {"mechanical", "ui", "interface", "rf"} or near_edge:
            return side, side
    if semantic_class == "core" or functional_group in {"control_processing", "memory", "timing"}:
        return "core", "core"
    # Ordinary module members/passives should not inherit edge preference merely
    # because their expert location happened to be close to an edge.
    return "free", "free"
...
def annotate_board_train(data: dict, *, overwrite: bool = True) -> dict:
    ...
    meta['weak_supervision_fields'] = ['functional_group', 'placement_role', 'align_group']
