...
def _edge_mixture_heatmap_payload(
    board_bbox: BBox,
    confidence: float,
    *,
    grid_x: int = 6,
    grid_y: int = 6,
    sigma_cells: float = 0.85,
    source: str = "edge_mixture",
) -> Dict[str, Any]:
    """Prior heatmap for unconstrained interface modules.

    Unknown-side interfaces should prefer *some* board edge, but should not get
    a center Gaussian.  This mixture keeps the prior leakage-safe while avoiding
    the center-pull failure mode seen in front-of-board connector placement.
    """
    acc = None
    for edge in ("left", "right", "top", "bottom"):
        arr = gaussian_heatmap_from_bbox(
            tuple(float(v) for v in board_bbox),
            tuple(float(v) for v in _edge_bbox(board_bbox, edge, 0.18)),
            int(grid_x),
            int(grid_y),
            float(sigma_cells),
            float(confidence),
    ...
    values = np.asarray(acc, dtype=np.float32)
    denom = float(values.sum())
    if denom > 1e-6:
        values = values / denom * float(confidence)
    return {
        "grid_x": int(grid_x),
        "grid_y": int(grid_y),
        "sigma_cells": float(sigma_cells),
        "values": values.reshape(int(grid_x), int(grid_y)).tolist(),
        "source": str(source or "edge_mixture"),
    }
...
def propose_prior_region_for_module(
    ...
    *,
    ...
    # Only a connector with an explicit edge constraint earns a narrow single-side
    # prior. Unknown-side interfaces should not receive a center Gaussian or a
    # shrunken-center broad box; instead, the active bbox is the whole board and
    # the heatmap is an edge mixture attached below.
    unconstrained_interface = family == "interface" and not (side and confirmed_edge)
    if unconstrained_interface:
        candidates = [
            {"bbox_mm": list(board_bbox), "score": 0.55, "reason": "interface_unconstrained_full_board"},
            *[
    ...
    prior = {
        ...
        "center_mm": None if unconstrained_interface else center,
        "confidence": float(conf),
        "source": "rule_v2_interface_unconstrained" if unconstrained_interface else f"rule_v2_{family}",
        "schema_version": RUNTIME_PRIOR_SCHEMA_VERSION,
        "leakage_safe": True,
        "family": family,
    }
    if unconstrained_interface:
        prior["heatmap_kind"] = "edge_mixture"
    if relative_prior:
        prior["relative_prior"] = relative_prior
    hint = {
        "version": 2,
        "schema_version": RUNTIME_SHAPE_HINT_SCHEMA_VERSION,
        "source": "prior_rule_v2_interface_unconstrained" if unconstrained_interface else f"prior_rule_v2_{family}",
        "leakage_safe": True,
        "module_center_mm": None if unconstrained_interface else center,
        "module_axis": axis,
...
def attach_prior_and_expert_regions(
    ...
    *,
    ...
    for m in modules:
        ...
        if str(prior.get("heatmap_kind") or "") == "edge_mixture":
            mm["prior_region_heatmap"] = _edge_mixture_heatmap_payload(
                board_bbox,
                float(prior.get("confidence", 0.35)),
                source=str(prior.get("source") or "rule_v2_interface_unconstrained"),
            )
        else:
            mm["prior_region_heatmap"] = _heatmap_payload(
                board_bbox,
                bbox,
                float(prior.get("confidence", 0.35)),
                source=str(prior.get("source") or "rule_v2"),
            )
