...
def _json_bool_value(value: Any) -> bool:
    if isinstance(value, bool):
        return bool(value)
    if isinstance(value, dict):
        if value.get("enabled") is not None:
            return _json_bool_value(value.get("enabled"))
        if value.get("fixed") is not None:
            return _json_bool_value(value.get("fixed"))
        if value.get("locked") is not None:
            return _json_bool_value(value.get("locked"))
        return True
    if isinstance(value, (list, tuple)):
        return len(value) >= 2
    if isinstance(value, str):
        return value.strip().lower() in {"1", "true", "yes", "y", "locked", "fixed", "freeze", "frozen"}
    try:
        return bool(value)
    except Exception:
        return False
...
def _json_fixed_flag(c: Dict[str, Any]) -> bool:
    for key in ("fixed", "locked", "is_fixed", "freeze", "frozen", "do_not_move"):
        value = _json_nested_value(c, key, None)
        if value not in (None, ""):
            return _json_bool_value(value)
    return False


def _json_get_nested_path(c: Dict[str, Any], *path: str) -> Any:
    cur: Any = c
    for key in path:
...
def _xy_from_value(value: Any) -> Optional[Tuple[float, float]]:
    if value in (None, ""):
        return None
    if isinstance(value, dict):
        ...
        for key in ("xy_mm", "center_mm", "position_mm", "pos_mm", "at_mm"):
...
def _json_fixed_xy(c: Dict[str, Any], is_fixed: bool) -> Optional[Tuple[float, float]]:
    candidates = [
        c.get("fixed_xy_mm"), c.get("fixed_position_mm"), c.get("fixed_pos_mm"), c.get("locked_xy_mm"),
        c.get("current_xy_mm"), c.get("current_position_mm"), c.get("current_pos_mm"),
        _json_get_nested_path(c, "fixed", "xy_mm"),
        _json_get_nested_path(c, "fixed", "center_mm"),
        _json_get_nested_path(c, "fixed", "position_mm"),
        _json_get_nested_path(c, "fixed", "at_mm"),
        _json_get_nested_path(c, "placement", "xy_mm"),
        _json_get_nested_path(c, "placement", "center_mm"),
        _json_get_nested_path(c, "placement", "position_mm"),
        _json_get_nested_path(c, "layout", "fixed_xy_mm"),
        _json_get_nested_path(c, "layout", "xy_mm"),
        _json_get_nested_path(c, "layout", "center_mm"),
        _json_get_nested_path(c, "layout", "position_mm"),
        _json_get_nested_path(c, "layout", "current_xy_mm"),
        _json_get_nested_path(c, "current", "xy_mm"),
        _json_get_nested_path(c, "current", "center_mm"),
        _json_get_nested_path(c, "current", "position_mm"),
    ]
    if isinstance(c.get("fixed"), (list, tuple, dict)):
        candidates.insert(0, c.get("fixed"))
    for value in candidates:
        xy = _xy_from_value(value)
        if xy is not None:
            return xy
    # When a user marks an already-placed DSL/exported JSON component as fixed,
    # allow the existing expert placement to become the runtime lock. Expert fields
    # remain ignored for non-fixed components and for normal runtime priors.
    if is_fixed and isinstance(c.get("expert"), dict):
        return _xy_from_value(c["expert"].get("xy_mm"))
    return None


def _rot_from_value(value: Any) -> Optional[int]:
    if value in (None, ""):
        return None
    if isinstance(value, dict):
        for key in ("rot", "rotation", "rot_deg", "angle", "angle_deg"):
            if value.get(key) not in (None, ""):
                return int(round(float(value.get(key)))) % 360
        return None
    if isinstance(value, (list, tuple)) and len(value) >= 3:
        return int(round(float(value[2]))) % 360
    try:
...
def _json_fixed_rot(c: Dict[str, Any], is_fixed: bool) -> int:
    candidates = [
        c.get("fixed_rot"), c.get("fixed_rot_deg"), c.get("locked_rot"), c.get("locked_rot_deg"),
        c.get("current_rot"), c.get("current_rot_deg"), c.get("current_rotation"),
        _json_get_nested_path(c, "fixed", "rot"),
        _json_get_nested_path(c, "fixed", "rot_deg"),
        _json_get_nested_path(c, "fixed", "rotation"),
        _json_get_nested_path(c, "placement", "rot"),
        _json_get_nested_path(c, "placement", "rot_deg"),
        _json_get_nested_path(c, "layout", "fixed_rot"),
        _json_get_nested_path(c, "layout", "rot"),
        _json_get_nested_path(c, "layout", "current_rot"),
        _json_get_nested_path(c, "current", "rot"),
        _json_get_nested_path(c, "current", "rot_deg"),
        _json_get_nested_path(c, "current", "rotation"),
        c.get("fixed") if isinstance(c.get("fixed"), (list, tuple, dict)) else None,
    ]
    for value in candidates:
        ...
        if rot is not None:
            return rot
    if is_fixed and isinstance(c.get("expert"), dict):
        rot = _rot_from_value(c["expert"].get("rot"))
        if rot is not None:
            return rot
    return 0


def _json_fixed_source(c: Dict[str, Any], is_fixed: bool, fixed_xy: Optional[Tuple[float, float]]) -> str:
    if not is_fixed:
        return ""
    for key in ("fixed_source", "lock_source"):
        value = _json_nested_value(c, key, None)
        if value not in (None, ""):
            return str(value)
...
def task_from_json(path: str, *, sequence_policy: str = "rebuild", load_expert: bool = False) -> Task:
    ...
    for c in data["components"]:
        ...
        is_fixed = _json_fixed_flag(c)
        fixed_xy = _json_fixed_xy(c, is_fixed)
        fixed_rot = _json_fixed_rot(c, is_fixed)
        comps.append(Component(
            ...
            external_io_role=str(c.get("external_io_role") or (c.get("semantic") or {}).get("external_io_role") or "none"),
            ...
            fixed=bool(is_fixed),
            fixed_xy=fixed_xy,
            fixed_rot=int(fixed_rot),
            fixed_source=_json_fixed_source(c, bool(is_fixed), fixed_xy),
            expert_xy=(
