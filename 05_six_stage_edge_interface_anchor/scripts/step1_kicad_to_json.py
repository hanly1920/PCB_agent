...
def footprint_center_pose(fp, typ: str, interface_pad_mode: str = "numeric") -> Tuple[float, float, float, float, float]:
    """Return (x, y, rot_ccw, local_cx, local_cy) for the component center.

    KiCad stores footprint ``at`` at the footprint origin; the model places the
    component bbox center. This mirrors build_layout_from_kicad() so locked
    KiCad footprints and DSL locks use exactly the same coordinate convention.
    """
    r = kicad_rot_to_ccw(float(getattr(fp, "rot", 0.0)))
    x = float(fp.at[0])
    y = float(fp.at[1])
    cx = cy = 0.0
    ...
    if bbox is not None:
        ...
        x += cx * c - cy * ss
        y += cx * ss + cy * c
    return float(x), float(y), float(r), float(cx), float(cy)
...
def component_center_from_footprint(fp, typ: str, interface_pad_mode: str = "numeric") -> tuple[float, float, float]:
    """Return (center_x_mm, center_y_mm, ccw_rot_deg) using the same convention
    as build_layout_from_kicad(). Fixed/locked components use this exact centre.
    """
...
def build_task_from_kicad(kicad_pcb: Path, grid_mm: float, default_size_mm=(2.0, 2.0),
    ...
    for fp in board.footprints:
        ...
        current_x, current_y, current_rot, current_cx, current_cy = footprint_center_pose(
            fp, typ, interface_pad_mode=interface_pad_mode
        )
        is_fixed = bool(getattr(fp, "locked", False))
        ...
        for p in fp.pads:
            ...
            pads.append({
                "net": net_name,
                "rel_mm": [float(p.at[0] - current_cx), float(p.at[1] - current_cy)],
        ...
        comp_obj = {
            ...
            "pads": pads,
            "allowed_sides": [],
            # Current KiCad pose is exported for DSL/runtime locks even when
            # the footprint is not locked in the source board.
            "current_xy_mm": [float(current_x), float(current_y)],
            "current_rot": float(current_rot),
            "fixed": bool(locked),
        }
        if locked:
            comp_obj.update({
                "locked": True,
                "fixed_xy_mm": [float(current_x), float(current_y)],
                "fixed_rot": float(current_rot),
                "fixed_source": "kicad_locked",
            })
...
def build_layout_from_kicad(kicad_pcb: Path, interface_pad_mode: str = "numeric") -> dict:
    ...
    for fp in board.footprints:
        ...
        x, y, r = component_center_from_footprint(fp, typ, interface_pad_mode=interface_pad_mode)
