...
try:
    ...
    def get_device_map(n_layers, devices):
        ...
        device_map = {int(d): [] for d in devices}
        for i in range(n_layers):
            device_map[int(devices[i % len(devices)])].append(i)
        return device_map

    def assert_device_map(device_map, num_blocks):
        blocks = []
        for layer_list in device_map.values():
            blocks.extend(layer_list)
        missing = set(range(num_blocks)) - set(blocks)
        extra = set(blocks) - set(range(num_blocks))
        if missing or extra:
            raise ValueError(f"Invalid device_map; missing={missing}, extra={extra}")
...
def load_tf_weights_in_gpt2(model, config, gpt2_checkpoint_path):
    """Load tf checkpoints in a pytorch model - simplified version"""
    # Skip TF loading for core functionality
    logger.warning("TF weight loading not implemented in this version")
    return model
...
class Attention(nn.Module):
    def __init__(self, nx, n_positions, config, scale=False, is_cross_attention=False):
        ...
        self.register_buffer(
            "bias", torch.tril(torch.ones((n_positions, n_positions), dtype=torch.uint8)).view(1, 1, n_positions, n_positions)
        )
    ...
    def prune_heads(self, heads):
        # Simplified: skip pruning for core functionality
        pass
...
class Block(nn.Module):
    def __init__(self, n_positions, config, scale=False):
        ...
        self.attn = Attention(hidden_size, n_positions, config, scale)
