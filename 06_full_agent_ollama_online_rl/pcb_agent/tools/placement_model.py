...
from .dsl_adapter import DSLTaskAdapter
...
class PlacementModelTool:
    ...
    def run(self, task_json_path: str | Path, dsl: LayoutDSL, *, candidate_count: int | None = None) -> ToolResult:
        try:
            ...
            task_path, adapter_observations, adapter_metrics = DSLTaskAdapter.materialize_task(Path(task_json_path), dsl)
            if self.config.mock:
                candidates=self._mock_candidates(task_path, count)
                return ToolResult.success(self.name,"Generated deterministic mock candidates",artifacts={"candidates":[c.model_dump(mode="json") for c in candidates],"task_json_path":str(task_path)},metrics={"candidate_count":len(candidates),**adapter_metrics},observations=adapter_observations+["Mock placement is intended for orchestration tests, not placement quality evaluation"])
            ...
            return ToolResult.success(self.name,"Placement inference completed",artifacts={"candidates":[c.model_dump(mode="json") for c in candidates],"checkpoint_meta":meta,"task_json_path":str(task_path)},metrics={"candidate_count":len(candidates),"complete_count":sum(c.complete for c in candidates),**adapter_metrics},observations=adapter_observations)
