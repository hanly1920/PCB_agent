...
from pcbplace.semantic_labels import annotate_board_train
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Annotate semantic labels for TRAIN mode. This may use expert layout geometry to create supervision labels.')
    ...
    if args.input:
        ...
        data = annotate_board_train(load_json(inp))
        save_json(data, out)
        print(f'[OK] train semantic: {inp} -> {out}')
        return 0
    ...
    for path in paths:
        try:
            ...
            save_json(annotate_board_train(load_json(path)), out)
            ok += 1
            print(f'[OK] {path} -> {out}')
