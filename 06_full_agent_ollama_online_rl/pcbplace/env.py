...
class Component:
    ...
    external_io_role: str = 'none'  # none|edge_hard|edge_soft|internal_connector
    ...
    # Runtime placement locks. Fixed components are inserted into the initial
    # PlacementEnv state, skipped by the action sequence, and their occupied
    # bboxes become hard keepout regions for all subsequently placed parts.
    fixed: bool = False
    fixed_xy: Optional[Tuple[float, float]] = None
    fixed_rot: int = 0
    fixed_source: str = ''
...
class PlacementObjectiveConfig:
    ...
    # Module-level floorplan term: operates on partial module bboxes, not just
    # per-component region hints.  Disabled by default for backward compatibility;
    # enable with --objective_module_floorplan_weight.
    module_floorplan_weight: float = 0.0
...
def _component_external_io_role(comp: Component) -> str:
    raw = str(getattr(comp, 'external_io_role', '') or '').strip().lower()
    if raw in {'edge_hard', 'edge_soft', 'internal_connector'}:
        return raw
    return 'none'


def _external_io_strength_multiplier(role: str) -> float:
...
def _component_must_touch_boundary(comp: Component) -> bool:
    ...
    role = _component_external_io_role(comp)
    # New edge_hard labels are trained as priors and masked in infer scoring.
    # They should not become legacy exact-touch constraints unless the JSON says so.
    ...
    if semantic in {'mechanical_edge_interface'}:
        return _component_constraint_level(comp) == 'hard'
    return bool(is_edge_required_type(comp.type) and _component_constraint_level(comp) == 'hard')
...
def _component_edge_preference_capable(comp: Component) -> bool:
    ...
    if role == 'internal_connector':
        return False
    if role in {'edge_hard', 'edge_soft'}:
        return True
    ...
    if ctype.startswith('conn_') or ctype.startswith('mech_') or ctype in {'ui_button', 'ui_led', 'antenna'}:
        return True
    if prefix_text in {'J', 'P', 'JP', 'CN', 'CON', 'CONN', 'RJ', 'X', 'XA', 'SW', 'LED', 'S', 'H', 'MH'}:
        return True
    return False
...
def _component_placement_role(comp: Component) -> str:
    ...
    if text in {'anchor_large', 'edge_anchor', 'main_anchor', 'member', 'internal_connector'}:
        return text
    if _component_external_io_role(comp) == 'internal_connector':
        return 'internal_connector'
...
def _component_is_fixed(comp: Component) -> bool:
    """Return True when a component is a runtime-fixed/locked placement."""
    for attr in ("fixed", "is_fixed", "locked", "freeze", "frozen"):
        try:
            if bool(getattr(comp, attr, False)):
                return True
        except Exception:
            continue
    return False
...
def _component_fixed_xy(comp: Component) -> Optional[Tuple[float, float]]:
    """Return the fixed component centre in board mm, if available."""
    for attr in ("fixed_xy", "fixed_xy_mm", "locked_xy", "locked_xy_mm"):
        value = getattr(comp, attr, None)
        if value in (None, ""):
            continue
        try:
...
def _component_fixed_rot(comp: Component) -> int:
    for attr in ("fixed_rot", "fixed_rot_deg", "locked_rot", "locked_rot_deg"):
        value = getattr(comp, attr, None)
        if value in (None, ""):
            continue
        try:
...
class PlacementEnv:
    ...
    def __init__(
        ...
        module_floorplan_separation_mm: float | None = None,
        ...
        objective_module_floorplan_weight: float | None = None,
        ...
        if objective_module_floorplan_weight is not None:
            module_floorplan_weight = float(objective_module_floorplan_weight)

        self.refs = [c.ref for c in task.components]
        self.comp_by_ref = {c.ref: c for c in task.components}
        self.fixed_refs = [ref for ref in self.refs if _component_is_fixed(self.comp_by_ref[ref])]
        self._fixed_ref_set = set(self.fixed_refs)
        dynamic_ref_set = {ref for ref in self.refs if ref not in self._fixed_ref_set}
        # Task.sequence remains the canonical all-component sequence for JSON/checkpoint
        # compatibility. PlacementEnv.sequence is the actual rollout sequence and
        # deliberately excludes fixed components. Missing dynamic refs are appended
        # defensively so stored legacy sequences cannot silently drop a component.
        seq = [ref for ref in list(task.sequence) if ref in dynamic_ref_set]
        seen_seq = set(seq)
        seq.extend(ref for ref in self.refs if ref in dynamic_ref_set and ref not in seen_seq)
        self.sequence = seq
        self.placeable_refs = list(self.sequence)
        ...
        self.module_floorplan_separation_mm = (
            float(module_floorplan_separation_mm)
            if module_floorplan_separation_mm is not None
            else max(2.0 * float(task.grid_mm), 1.5)
        )
        self.module_floorplan_overlap_scale = float(module_floorplan_overlap_scale)
        self.module_floorplan_channel_scale = float(module_floorplan_channel_scale)
        self.module_floorplan_compact_scale = float(module_floorplan_compact_scale)
        self.module_floorplan_region_scale = float(module_floorplan_region_scale)
        ...
        self.objective_cfg = PlacementObjectiveConfig(
            ...
            module_floorplan_weight=float(module_floorplan_weight),
        ...
        self._external_io_roles = {ref: _component_external_io_role(self.comp_by_ref[ref]) for ref in self.refs}
    ...
    @staticmethod
    def _bbox_union_many(bboxes: List[Tuple[float, float, float, float]]) -> Optional[Tuple[float, float, float, float]]:
        if not bboxes:
            return None
        return (
            min(float(bb[0]) for bb in bboxes),
            min(float(bb[1]) for bb in bboxes),
            max(float(bb[2]) for bb in bboxes),
            max(float(bb[3]) for bb in bboxes),
        )
    ...
    @staticmethod
    def _bbox_gap(bb1: Tuple[float, float, float, float], bb2: Tuple[float, float, float, float]) -> float:
        dx = max(0.0, max(float(bb2[0]) - float(bb1[2]), float(bb1[0]) - float(bb2[2])))
        dy = max(0.0, max(float(bb2[1]) - float(bb1[3]), float(bb1[1]) - float(bb2[3])))
        if dx > 0.0 and dy > 0.0:
            return math.hypot(dx, dy)
        return max(dx, dy)

    @staticmethod
    def _bbox_overlap_area(bb1: Tuple[float, float, float, float], bb2: Tuple[float, float, float, float]) -> float:
        ox = max(0.0, min(float(bb1[2]), float(bb2[2])) - max(float(bb1[0]), float(bb2[0])))
        oy = max(0.0, min(float(bb1[3]), float(bb2[3])) - max(float(bb1[1]), float(bb2[1])))
        return ox * oy

    def _module_pair_floorplan_scale(self, ref: str, peer_refs: List[str]) -> float:
        """Reduce separation pressure for tightly connected / critical cross-module peers.

        Module-level floorplanning should reserve channels between unrelated modules,
        but it must not push decoupling/support/critical-neighbor modules too far away.
        """
        if not peer_refs:
            return 1.0
        max_conn = 0.0
        critical = set(getattr(self, '_critical_neighbors', {}).get(ref, ()) or ())
        for pref in peer_refs:
            try:
                max_conn = max(max_conn, float(self._pair_conn_weight(ref, pref)))
            except Exception:
                pass
            if pref in critical:
                max_conn = max(max_conn, 1.0)
        if max_conn >= 1.0:
            return 0.35
        if max_conn >= 0.25:
            return 0.65
    ...
    def _placed_module_bboxes(self, placed_bbs: Optional[Dict[str, Tuple[float, float, float, float]]] = None) -> Dict[str, Tuple[Tuple[float, float, float, float], List[str]]]:
        placed_bbs = placed_bbs or {r: self._ref_bbox(r, *self.placed[r]) for r in self.placed_order if r in self.placed}
        grouped: Dict[str, List[Tuple[str, Tuple[float, float, float, float]]]] = {}
        for r, bb in placed_bbs.items():
            mid = str(getattr(self, '_module_ids', {}).get(r, '') or '').strip()
            if not mid:
                continue
            grouped.setdefault(mid, []).append((r, bb))
        out: Dict[str, Tuple[Tuple[float, float, float, float], List[str]]] = {}
        for mid, vals in grouped.items():
            bb = self._bbox_union_many([v[1] for v in vals])
            if bb is not None:
                out[mid] = (bb, [v[0] for v in vals])
        return out

    def _module_bbox_region_penalty(self, ref: str, module_bb: Tuple[float, float, float, float]) -> float:
        rb = self._module_region_bbox_for_ref(ref)
        conf = self._module_region_confidence_for_ref(ref)
        if rb is None or conf <= 0.0:
            return 0.0
        margin = float(self.module_region_margin_mm)
        x0, y0, x1, y1 = [float(v) for v in rb]
        ...
        over_x = max(0.0, x0 - a) + max(0.0, c - x1)
        over_y = max(0.0, y0 - b) + max(0.0, d - y1)
        return float(conf * self.module_floorplan_region_scale * ((over_x + over_y) / max(1e-6, self.board_diag)) ** 2)

    def _module_bbox_compact_penalty(self, ref: str, module_bb: Tuple[float, float, float, float]) -> float:
        mid = str(getattr(self, '_module_ids', {}).get(ref, '') or '').strip()
        if not mid:
            return 0.0
        members = [r for r in self.refs if str(getattr(self, '_module_ids', {}).get(r, '') or '').strip() == mid]
        if len(members) < 3:
            return 0.0
        area = self._bbox_area(module_bb)
        if area <= 0.0:
            return 0.0
        rb = self._module_region_bbox_for_ref(ref)
        if rb is not None:
            target = max(1e-6, self._bbox_area(tuple(float(v) for v in rb)))
            # Allow the learned module bbox to be a little larger than the coarse prior;
            # penalize only excessive growth so anchors/support parts can still breathe.
            excess = max(0.0, area - 1.25 * target) / max(1e-6, self.board_area)
            return float(self.module_floorplan_compact_scale * excess)
        return float(self.module_floorplan_compact_scale * 0.25 * (math.sqrt(area) / max(1e-6, self.board_diag)) ** 2)

    def _module_floorplan_penalty_for_candidate(self, ref: str, bb: Tuple[float, float, float, float]) -> float:
        mid = str(getattr(self, '_module_ids', {}).get(ref, '') or '').strip()
        if not mid:
            return 0.0
        same_bbs = [self._ref_bbox(r, *self.placed[r]) for r in self.placed_order if str(getattr(self, '_module_ids', {}).get(r, '') or '').strip() == mid]
        module_bb = self._bbox_union_many([bb] + same_bbs)
        if module_bb is None:
            return 0.0
        pen = self._module_bbox_region_penalty(ref, module_bb) + self._module_bbox_compact_penalty(ref, module_bb)

        others = self._placed_module_bboxes()
        pair_pen = 0.0
        pair_count = 0
        cand_area = max(1e-6, self._bbox_area(module_bb))
        sep_base = max(0.0, float(self.module_floorplan_separation_mm))
        for omid, (obb, orefs) in others.items():
            if omid == mid:
                continue
            other_area = max(1e-6, self._bbox_area(obb))
            overlap = self._bbox_overlap_area(module_bb, obb)
            overlap_ratio = overlap / max(1e-6, min(cand_area, other_area))
            gap = self._bbox_gap(module_bb, obb)
            scale = self._module_pair_floorplan_scale(ref, orefs)
            sep = sep_base * scale
            channel = 0.0
            if sep > 1e-6 and gap < sep:
                channel = ((sep - gap) / sep) ** 2
            pair_pen += scale * (
                float(self.module_floorplan_overlap_scale) * overlap_ratio
                + float(self.module_floorplan_channel_scale) * channel
            )
            pair_count += 1
        if pair_count:
            pen += pair_pen / float(pair_count)
        return float(max(0.0, pen))

    def _module_floorplan_penalty_for_layout(self, placed_bbs: Dict[str, Tuple[float, float, float, float]]) -> float:
        modules = self._placed_module_bboxes(placed_bbs)
        if not modules:
            return 0.0
        pen = 0.0
        for mid, (mbb, refs) in modules.items():
            ref0 = refs[0]
            pen += self._module_bbox_region_penalty(ref0, mbb)
            pen += self._module_bbox_compact_penalty(ref0, mbb)
        ...
        for i in range(len(mids)):
            mi = mids[i]
            bbi, refsi = modules[mi]
            ai = max(1e-6, self._bbox_area(bbi))
            ref0 = refsi[0]
            for j in range(i + 1, len(mids)):
                mj = mids[j]
                bbj, refsj = modules[mj]
                aj = max(1e-6, self._bbox_area(bbj))
                overlap = self._bbox_overlap_area(bbi, bbj)
                overlap_ratio = overlap / max(1e-6, min(ai, aj))
                gap = self._bbox_gap(bbi, bbj)
                scale = min(self._module_pair_floorplan_scale(ref0, refsj), self._module_pair_floorplan_scale(refsj[0], refsi))
                sep = sep_base * scale
                channel = 0.0
                if sep > 1e-6 and gap < sep:
                    channel = ((sep - gap) / sep) ** 2
    ...
    def objective_delta_mask(self, ref: str) -> Dict[str, np.ndarray]:
        ...
        module_floorplan = np.zeros_like(region)
        ...
        for ri, rot in enumerate(self.rotations):
            ...
            for xi in range(w_cells):
                for yi in range(h_cells):
                    ...
                    module_floorplan[ri, xi, yi] = self._module_floorplan_penalty_for_candidate(ref, bb)
        ...
        total = (
            ...
            + self.objective_cfg.module_floorplan_weight * module_floorplan
        ...
        return {
            ...
            'module_floorplan': module_floorplan,
            'conn': conn,
            'align': align,
            'group': group,
            'anchor': anchor,
    ...
    def reset(self):
        ...
        self._place_fixed_components_at_reset()
        self.prev_obj = self._objective()
        return self.observe()

    def is_ref_fixed(self, ref: str) -> bool:
        return str(ref) in getattr(self, '_fixed_ref_set', set())

    def fixed_placement_for_ref(self, ref: str) -> Optional[Tuple[float, float, int]]:
        comp = self.comp_by_ref.get(str(ref))
        if comp is None or not _component_is_fixed(comp):
            return None
        xy = _component_fixed_xy(comp)
        if xy is None:
            return None
        return (float(xy[0]), float(xy[1]), int(_component_fixed_rot(comp)))

    def fixed_placement_map(self) -> Dict[str, Tuple[float, float, int]]:
        out: Dict[str, Tuple[float, float, int]] = {}
        for ref in getattr(self, 'fixed_refs', []):
            p = self.fixed_placement_for_ref(ref)
            if p is not None:
                out[ref] = p
        return out

    def dynamic_placed_count(self) -> int:
        return sum(1 for ref in self.sequence if ref in self.placed)

    def dynamic_expected_count(self) -> int:
        return int(len(self.sequence))

    def total_expected_count(self) -> int:
        return int(len(self.refs))

    def _place_fixed_components_at_reset(self) -> None:
        for ref in getattr(self, 'fixed_refs', []):
            comp = self.comp_by_ref.get(ref)
            xy = _component_fixed_xy(comp) if comp is not None else None
            if xy is None:
                raise ValueError(
                    f"Fixed component {ref} requires a fixed_xy/fixed_xy_mm/placement.xy_mm centre. "
                    f"For KiCad locked footprints, rerun step1_kicad_to_json so locked parts export fixed_xy_mm."
                )
            rot = int(_component_fixed_rot(comp))
            placement = (float(xy[0]), float(xy[1]), rot)
            bb = self._ref_bbox(ref, *placement)
            if not self._inside(bb):
                raise ValueError(f"Fixed component {ref} is outside board bbox: placement={placement} bbox={bb}")
            if self._violates_spacing(bb):
                raise ValueError(
                    f"Fixed component {ref} overlaps or violates spacing with an earlier fixed component; "
                    f"placement={placement} bbox={bb}"
                )
            self.placed[ref] = placement
    ...
    def _soft_edge_preference_sides(self, ref: str) -> List[str]:
        ...
        comp = self.comp_by_ref[ref]
        if self._external_io_roles.get(ref) == 'internal_connector':
            return []
        sides: List[str] = []
        region = self._region_targets.get(ref, 'free')
        if region in {'edge_left', 'edge_right', 'edge_top', 'edge_bottom'}:
    ...
    def _component_edge_band_width_mm(self, ref: str) -> float:
        comp = self.comp_by_ref.get(ref)
        if comp is None:
            return self._edge_band_width_mm()
        try:
            cw, ch = float(comp.size_mm[0]), float(comp.size_mm[1])
        except Exception:
            cw, ch = 0.0, 0.0
        role = self._external_io_roles.get(ref, 'none')
        if role in {'edge_hard', 'edge_soft', 'internal_connector'}:
            return float(max(self.task.grid_mm, 2.5, 0.5 * max(cw, ch) + 1.0))
        return self._edge_band_width_mm()
    ...
    def _edge_target_clearance_mm(self, ref: str, side: str) -> float:
        _ = side
        band = float(self._component_edge_band_width_mm(ref))
    ...
    def _within_edge_band(self, bb: Tuple[float, float, float, float], sides: List[str], ref: Optional[str] = None) -> bool:
        if not sides:
            return False
        band = self._component_edge_band_width_mm(ref) if ref else self._edge_band_width_mm()
    ...
    def action_mask_and_bias(self, ref: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        eps = self.edge_eps_mm
        band = self._component_edge_band_width_mm(ref)
        ...
        for ri, rot in enumerate(self.rotations):
            ...
            if edge_bonus_sides:
                ...
                if edge_terms:
                    ...
                    denom = max(self.task.grid_mm, self._component_edge_band_width_mm(ref), 1e-6)
    ...
    def _objective(self) -> float:
        ...
        module_floorplan_pen = self._module_floorplan_penalty_for_layout(placed_bbs)
        ...
        return float(
            ...
            + self.objective_cfg.module_floorplan_weight * module_floorplan_pen
