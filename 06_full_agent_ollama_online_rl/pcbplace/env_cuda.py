...
def _compute_align_delta(
    ...
    axis = str(getattr(env, '_align_axes', {}).get(ref, 'auto') or 'auto').lower()
    if axis == 'x':
        vals = dx
    elif axis == 'y':
        vals = dy
    ...
    for pref in peers:
        ...
        peer_axis = str(getattr(env, '_align_axes', {}).get(pref, axis) or axis).lower()
        use_axis = axis if axis in {'x', 'y'} else peer_axis
        if use_axis == 'x':
            new_ref_val = torch.abs(torch.tensor(px, device=device) - Xc) / bw
        elif use_axis == 'y':
            new_ref_val = torch.abs(torch.tensor(py, device=device) - Yc) / bh
        else:
        ...
        if old_targets:
            ov = torch.tensor(
                [
                    (abs(px - cur_xy[r][0]) / bw) if use_axis == 'x'
                    else ((abs(py - cur_xy[r][1]) / bh) if use_axis == 'y' else min(abs(px - cur_xy[r][0]) / bw, abs(py - cur_xy[r][1]) / bh))
...
def _module_floorplan_pair_scale(env: PlacementEnv, ref: str, peer_refs: List[str]) -> float:
    if not peer_refs:
        return 1.0
    max_conn = 0.0
    critical = set(getattr(env, '_critical_neighbors', {}).get(ref, ()) or ())
    for pref in peer_refs:
        try:
            max_conn = max(max_conn, float(env._pair_conn_weight(ref, pref)))
        except Exception:
            pass
        if pref in critical:
...
def _compute_module_floorplan(
    ...
    """Module-level floorplan penalty for candidate partial module bboxes.

    Unlike ``module_region`` (per-component inside a broad prior bbox), this term
    forms the candidate module bbox by unioning the current candidate with already
    placed members of the same module.  It then penalizes:
      * overlap with other already-placed module bboxes,
      * missing channel/gap between unrelated modules,
      * excessive module bbox growth,
      * module bbox leakage outside the coarse prior region.
    """
    zero = torch.zeros_like(a + b)
    mid = str(getattr(env, '_module_ids', {}).get(ref, '') or '').strip()
    if not mid:
        return zero
    placed_order = list(getattr(env, 'placed_order', []) or [])
    if not placed_order:
        # The first component of a module has no inter-module relation yet, but
        # still benefits from module bbox vs prior-region supervision below.
        same_bbs = []
    else:
        same_bbs = [env._ref_bbox(r, *env.placed[r]) for r in placed_order if str(getattr(env, '_module_ids', {}).get(r, '') or '').strip() == mid]
    ...
    D = d_t
    if same_bbs:
        sb = torch.tensor(same_bbs, device=device, dtype=a.dtype)
        A = torch.minimum(A, sb[:, 0].min())
        B = torch.minimum(B, sb[:, 1].min())
        C = torch.maximum(C, sb[:, 2].max())
        D = torch.maximum(D, sb[:, 3].max())

    diag = max(1e-6, float(board_diag))
    pen = zero

    # Module bbox vs prior region.  This is intentionally separate from the
    # per-component module_region term: it teaches the whole group footprint.
    ...
    if rb is not None and conf > 0.0:
        ...
        over_x = torch.clamp(float(x0) - A, min=0) + torch.clamp(C - float(x1), min=0)
        over_y = torch.clamp(float(y0) - B, min=0) + torch.clamp(D - float(y1), min=0)
        pen = pen + float(getattr(env, 'module_floorplan_region_scale', 0.60)) * conf * ((over_x + over_y) / diag) ** 2

    # Compactness: only mild, and only for modules with at least three members.
    members = [r for r in getattr(env, 'refs', []) if str(getattr(env, '_module_ids', {}).get(r, '') or '').strip() == mid]
    if len(members) >= 3:
        area = torch.clamp(C - A, min=0) * torch.clamp(D - B, min=0)
        rb_area = 0.0
        if rb is not None:
            rb_area = max(0.0, (float(rb[2]) - float(rb[0])) * (float(rb[3]) - float(rb[1])))
        if rb_area > 1e-6:
            excess = torch.clamp(area - 1.25 * float(rb_area), min=0) / max(1e-6, float(env.board_area))
            pen = pen + float(getattr(env, 'module_floorplan_compact_scale', 0.20)) * excess
        else:
            pen = pen + 0.25 * float(getattr(env, 'module_floorplan_compact_scale', 0.20)) * (torch.sqrt(torch.clamp(area, min=0)) / diag) ** 2

    # Build other module bboxes from placed components.
    module_groups: Dict[str, List[Tuple[str, Tuple[float, float, float, float]]]] = {}
    for r in placed_order:
        omid = str(getattr(env, '_module_ids', {}).get(r, '') or '').strip()
        if not omid or omid == mid:
            continue
        module_groups.setdefault(omid, []).append((r, env._ref_bbox(r, *env.placed[r])))
    if not module_groups:
        return torch.clamp(pen, min=0.0)

    other_bbs = []
    pair_scales = []
    for omid, vals in module_groups.items():
        refs = [v[0] for v in vals]
        bbs = [v[1] for v in vals]
        obb = (
            min(float(bb[0]) for bb in bbs),
            min(float(bb[1]) for bb in bbs),
            max(float(bb[2]) for bb in bbs),
            max(float(bb[3]) for bb in bbs),
        )
        other_bbs.append(obb)
        pair_scales.append(_module_floorplan_pair_scale(env, ref, refs))
    if not other_bbs:
        return torch.clamp(pen, min=0.0)

    ob = torch.tensor(other_bbs, device=device, dtype=a.dtype)
    sc = torch.tensor(pair_scales, device=device, dtype=a.dtype)
    ox0, oy0, ox1, oy1 = ob[:, 0], ob[:, 1], ob[:, 2], ob[:, 3]
    cand_area = torch.clamp(C - A, min=0) * torch.clamp(D - B, min=0)
    other_area = torch.clamp(ox1 - ox0, min=0) * torch.clamp(oy1 - oy0, min=0)

    overlap_x = torch.clamp(torch.minimum(C.unsqueeze(-1), ox1) - torch.maximum(A.unsqueeze(-1), ox0), min=0)
    overlap_y = torch.clamp(torch.minimum(D.unsqueeze(-1), oy1) - torch.maximum(B.unsqueeze(-1), oy0), min=0)
    overlap = overlap_x * overlap_y
    overlap_ratio = overlap / torch.clamp(torch.minimum(cand_area.unsqueeze(-1), other_area), min=1e-6)

    dx_gap = torch.clamp(torch.maximum(ox0 - C.unsqueeze(-1), A.unsqueeze(-1) - ox1), min=0)
    dy_gap = torch.clamp(torch.maximum(oy0 - D.unsqueeze(-1), B.unsqueeze(-1) - oy1), min=0)
    both = (dx_gap > 0) & (dy_gap > 0)
    gap = torch.where(both, torch.sqrt(dx_gap * dx_gap + dy_gap * dy_gap), torch.maximum(dx_gap, dy_gap))

    sep = max(0.0, float(getattr(env, 'module_floorplan_separation_mm', 0.0))) * sc
    channel = torch.where(sep > 1e-6, (torch.clamp(sep - gap, min=0) / torch.clamp(sep, min=1e-6)) ** 2, torch.zeros_like(gap))
    pair_pen = sc * (
        float(getattr(env, 'module_floorplan_overlap_scale', 1.0)) * overlap_ratio
        + float(getattr(env, 'module_floorplan_channel_scale', 0.65)) * channel
    )
    pen = pen + pair_pen.mean(dim=-1)
    return torch.clamp(pen, min=0.0)
...
def objective_delta_mask_cuda(
    ...
    module_floorplan = torch.zeros_like(region)
    ...
    # Most rotation-dependent objective maps depend only on the rotated bbox
    # size (wm, hm), not on the absolute angle.  Reuse 0/180 and 90/270 maps;
    # keep orientation separate because it depends on the true rotation value.
    rot_cache: Dict[Tuple[float, float], Dict[str, torch.Tensor]] = {}
    for ri, rot in enumerate(env.rotations):
        ...
        if key not in rot_cache:
            ...
            cached['region'] = _coerce_wh_map(_compute_region(env, ref, a, b_t, c_t, d_t, Xc, Yc, board_diag), w_cells, h_cells, f'region[{ri}]')
            cached['module_region'] = _coerce_wh_map(_compute_module_region(env, ref, a, b_t, c_t, d_t, board_diag), w_cells, h_cells, f'module_region[{ri}]')
            cached['module_floorplan'] = _coerce_wh_map(_compute_module_floorplan(env, ref, a, b_t, c_t, d_t, board_diag, device), w_cells, h_cells, f'module_floorplan[{ri}]')
            cached['edge_clearance'] = _coerce_wh_map(_compute_edge_clearance(env, ref, a, b_t, c_t, d_t), w_cells, h_cells, f'edge_clearance[{ri}]')
            if n_placed:
                cached['soft_spacing'] = _coerce_wh_map(_compute_soft_spacing(env, ref, a, b_t, c_t, d_t, pd), w_cells, h_cells, f'soft_spacing[{ri}]')
            else:
                cached['soft_spacing'] = zero_2d
            ...
            cached['neatness'] = _coerce_wh_map(0.50 * line_neat_2d + 0.20 * wb + 0.30 * wf, w_cells, h_cells, f'neatness[{ri}]')
            rot_cache[key] = cached

        cached = rot_cache[key]
        region[ri] = cached['region']
        module_region[ri] = cached['module_region']
        module_floorplan[ri] = cached['module_floorplan']
        edge_clearance[ri] = cached['edge_clearance']
        soft_spacing[ri] = cached['soft_spacing']
        neatness[ri] = cached['neatness']
    ...
    objective_terms = {
        ...
        'region': region, 'module_region': module_region, 'module_floorplan': module_floorplan, 'conn': conn,
    ...
    region = objective_terms['region']; module_region = objective_terms['module_region']; module_floorplan = objective_terms['module_floorplan']; conn = objective_terms['conn']
    ...
    duplicated_action_terms = (
        ...
        + cfg.module_floorplan_weight * module_floorplan
    ...
    for ri, (ca, sa) in enumerate(rotation_info):
        if pads_by_net:
            for net, rels in pads_by_net.items():
                ...
                # Fixed rotation: the candidate component pad bbox is just
                # center + scalar min/max offsets. Avoid building per-pad stacks
                # and launching min/max kernels for every net/rotation.
                dx_min = float('inf')
                dx_max = float('-inf')
                dy_min = float('inf')
                dy_max = float('-inf')
...
def _compute_nslw_delta_cuda(
    ...
    blocker_bbs_by_target: List[torch.Tensor] = []
    if n_blockers > 0:
        for target_idx in range(n_blockers):
            if n_blockers > 1:
                blocker_mask = torch.ones(n_blockers, device=device, dtype=torch.bool)
                blocker_mask[target_idx] = False
                blocker_bbs_by_target.append(placed_bbs_t[blocker_mask])
            else:
                blocker_bbs_by_target.append(placed_bbs_t[:0])
        ...
        for dx, dy in cand_pads:
            ...
            for pref, qx, qy in placed_pins:
                ...
                if placed_edge_mask & (edge_bits["left"] | edge_bits["right"]):
                    ...
                    if n_blockers > 1:
                        bb = blocker_bbs_by_target[target_idx]
                        if bb.numel() > 0:
                            a = bb[:, 0].view(-1, 1, 1)
                            b = bb[:, 1].view(-1, 1, 1)
                            cbb = bb[:, 2].view(-1, 1, 1)
...
def _compute_action_dynamic_maps_cuda(env: PlacementEnv, ref: str, device: torch.device) -> Dict[str, torch.Tensor]:
    ...
    if hasattr(env, '_component_edge_band_width_mm'):
        band = max(2.0 * grid, float(env._component_edge_band_width_mm(ref)))
    else:
...
def action_mask_and_bias_cuda(env: PlacementEnv, ref: str, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if current_align_group:
        ...
        if axis == 'x':
            ...
            if valid_x:
                align_score[torch.tensor(valid_x, device=device, dtype=torch.long), :] += 0.5
            if valid_y:
                align_score[:, torch.tensor(valid_y, device=device, dtype=torch.long)] += 0.5
    ...
    for ri, rot in enumerate(env.rotations):
        ...
        if edge_bonus_sides:
            ...
            if edge_terms:
                ...
                denom = max(float(env.task.grid_mm), float(env._component_edge_band_width_mm(ref) if hasattr(env, '_component_edge_band_width_mm') else env._edge_band_width_mm()), 1e-6)
