...
def _sexpr_bool(items: List[Any], default: bool = True) -> bool:
    """Parse KiCad boolean-ish subexpressions such as (locked yes)."""
    if len(items) < 2:
        return bool(default)
    value = str(items[1]).strip().lower()
    if value in {"0", "false", "no", "off", "none"}:
        return False
    if value in {"1", "true", "yes", "on", "locked"}:
        return True
    return bool(default)
...
def parse_kicad_pcb(text: str) -> Board:
    ...
    def walk_fp(node: Any):
        ...
        if node[0] in ('footprint', 'module'):
            ...
            for item in node[2:]:
                if item == 'locked' or (isinstance(item, list) and item and item[0] == 'locked'):
                    locked = True
                if isinstance(item, list) and item:
                    ...
                    if item[0] == 'locked':
                        locked = _sexpr_bool(item, default=True)
                    if item[0] == 'unlocked':
                        if _sexpr_bool(item, default=True):
                            locked = False
            ...
            if ref:
                footprints.append(Footprint(ref=ref, footprint=fp_name, at=at, rot=rot, locked=bool(locked), pads=pads, bbox_local=local_bbox, bbox_courtyard=crtyd_bbox, bbox_pads=pad_bbox))
        for ch in node[1:]:
            walk_fp(ch)
