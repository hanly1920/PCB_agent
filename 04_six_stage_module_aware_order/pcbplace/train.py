...
def rollout_suffix_loss_batched_boards(
    ...
    *,
    ...
    suffix_steps = [max(1, int(T) - int(pfx)) for T, pfx in zip(T_list, prefix_lens)]
    ...
    while True:
        candidate_indices = [i for i in range(B) if (cur_t[i] < T_list[i]) and (not envs[i].done())]
    ...
    if phase == 'main':
        while True:
            ...
            for batch_idx in batch_indices:
                ...
                env = runtime_cache.make_env_at_suffix(batch_idx, k_eff)
