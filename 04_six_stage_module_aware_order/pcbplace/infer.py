...
def _beam_env_is_complete(env: PlacementEnv) -> bool:
    ...
    expected = int(len(env.sequence))
    placed_count = int(len(env.placed))
    return bool(
        not env.terminated
        and placed_count == expected
        and int(env.t) >= expected
    )
...
def infer_layout(
    ...
    *,
    ...
    model, meta, teacher, region_cfg = load_model(
        ckpt_path,
        device=device,
    ...
    if not complete:
        ...
        elif placed_count != expected_count:
            failure_reason = "incomplete_layout"
        else:
            failure_reason = "invalid_incomplete_layout"

    changed_refs = sorted([ref for ref in final_placed if final_placed.get(ref) != raw_placed.get(ref)])
